...
public final class Anim {
    ...
    public static float fade(long startMillis, long durationMillis) {
        ...
        return Mth.clamp((now() - startMillis) / (float) durationMillis, 0f, 1f);
    }
     ...
     */
    public static void spinner(GuiGraphicsExtractor context, int cx, int cy, int radius, int color) {
        int dots = 8;
        ...
        for (int i = 0; i < dots; i++) {
            ...
            float alpha = Mth.clamp(1f - dist / dots, 0.15f, 1f);
    ...
    public static final class Tween {
        ...
        public float approach(float target, float speed) {
            ...
            lastNanos = nowNanos;
            dt = Mth.clamp(dt, 0f, 0.1f); // guard against pauses / first frame
