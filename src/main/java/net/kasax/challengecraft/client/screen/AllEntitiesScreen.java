...
public class AllEntitiesScreen extends CollectionScreen {
    public AllEntitiesScreen(List<EntityType<?>> entities, int currentIndex) {
        super(Component.translatable("challengecraft.all_entities_list.title"), toRows(entities), currentIndex);
    }

    private static List<Row> toRows(List<EntityType<?>> entities) {
        List<Row> rows = new ArrayList<>();
        for (EntityType<?> type : entities) {
            rows.add(new Row(Chal_23_AllEntities.getIcon(type), type.getDescription(), List.of()));
        }
        return rows;
    }
}
