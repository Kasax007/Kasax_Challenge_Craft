...
public class ForceItemTeamScreen extends Screen {
    ...
    public ForceItemTeamScreen() {
        super(Text.translatable("challengecraft.fib.team_screen.title"));
    }
    ...
    protected void init() {
        ...
        if (state == ForceItemBattleSavedData.STATE_IDLE) {
            ...
            for (int i = 0; i < teams.length; i++) {
                ...
                addDrawableChild(new CraftButton(panelX + 8 + i * (buttonW + 4), teamY, buttonW, 20,
            ...
            addDrawableChild(new CraftButton(panelX + 8, teamY + 24, (panelW - 20) / 2, 20,
                    Text.translatable("challengecraft.fib.team_screen.solo"), CraftButton.Style.NEUTRAL,
                    btn -> ClientPlayNetworking.send(new ForceItemActionPacket(ForceItemActionPacket.Action.LEAVE_TEAM, -1))));
            addDrawableChild(new CraftButton(panelX + 12 + (panelW - 20) / 2, teamY + 24, (panelW - 20) / 2, 20,
                    Text.translatable("challengecraft.fib.team_screen.start"), CraftButton.Style.PRIMARY,
            ...
            addDrawableChild(new CraftButton(panelX + 8, panelY + 40, panelW - 16, 20,
                    Text.translatable("challengecraft.fib.team_screen.use_joker"), CraftButton.Style.PRIMARY,
                    btn -> ClientPlayNetworking.send(new ForceItemActionPacket(ForceItemActionPacket.Action.USE_JOKER, -1))));
        } else {
            addDrawableChild(new CraftButton(panelX + 8, panelY + 40, panelW - 16, 20,
                    Text.translatable("challengecraft.fib.team_screen.show_results"), CraftButton.Style.PRIMARY,
                    btn -> ClientPlayNetworking.send(new ForceItemActionPacket(ForceItemActionPacket.Action.TRIGGER_RESULTS, -1))));
        }

        addDrawableChild(new CraftButton(width / 2 - 60, panelY + panelH + 8, 120, 20,
                Text.translatable("gui.done"), CraftButton.Style.NEUTRAL,
                btn -> close()));
    }
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.drawCenteredTextWithShadow(textRenderer, title, width / 2, panelY + 6, CraftUI.TEXT_PRIMARY);

        ForceItemSyncPacket state = ForceItemClientState.get();
        Text status = switch (state.state()) {
            case ForceItemBattleSavedData.STATE_RUNNING -> {
                long seconds = ForceItemClientState.remainingTicksNow() / 20L;
                yield Text.translatable("challengecraft.fib.team_screen.running",
                        String.format("%d:%02d", seconds / 60, seconds % 60));
            }
            case ForceItemBattleSavedData.STATE_ENDED -> Text.translatable("challengecraft.fib.team_screen.ended");
            default -> Text.translatable("challengecraft.fib.team_screen.idle");
        };
        ctx.drawCenteredTextWithShadow(textRenderer, status, width / 2, panelY + 20, CraftUI.WARNING);
        ...
        if (ownEntry != null && !idle) {
            ...
            Item current = ownEntry.itemId().isEmpty() ? Items.AIR : Registries.ITEM.get(Identifier.of(ownEntry.itemId()));
            if (current != Items.AIR) {
                CraftUI.iconTileItem(ctx, new ItemStack(current), panelX + 8, y, 18, CraftUI.GOLD);
                ctx.drawText(textRenderer, Text.translatable("challengecraft.fib.team_screen.current", current.getName()),
            ...
            ctx.drawText(textRenderer,
                    Text.translatable("challengecraft.fib.team_screen.history", ownEntry.collected().size()),
        ...
        CraftUI.sectionHeader(ctx, textRenderer, Text.translatable("challengecraft.fib.team_screen.players"),
        ...
        for (ForceItemSyncPacket.PlayerEntry entry : state.players()) {
            ...
            ctx.drawText(textRenderer, Text.of(line), panelX + 12, y, color, false);
            y += 12;
        }
    }
}
